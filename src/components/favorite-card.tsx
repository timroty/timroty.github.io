...
export default function FavoriteCard({ title, tags, date, link, author }: Props) {
  ...
  return (
    <div className="px-4 rounded-md border border-slate-200 dark:border-border bg-secondary max-w-screen-sm">
      ...
      <div className={`rounded-sm border pl-1 pr-1 max-w-fit text-xs mt-2 mb-2 ${getPillColor(tags[0])}`}>
        {tags[0]}
      </div>
    </div>
  );
